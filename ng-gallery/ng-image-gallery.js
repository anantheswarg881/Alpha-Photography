			...
			link: {
				pre : function (scope, element, attributes){
					var autoFitBubbles = function(){
						...
						if ($window.width < 480)
						{
						    scope.bubbleSize = scope.bubbleSize / 2;
						}
		...
		return {
			...
			template : 	'<div class="ng-image-gallery img-move-dir-{{_imgMoveDirection}}" ng-class="{inline:inline}" ng-hide="images.length == 0">'+
							...
							'<div ng-if="thumbnails && !inline" class="ng-image-gallery-thumbnails">' +
 								'<div class="thumb" ng-repeat="image in images track by image.id" ng-click="methods.open($index);" show-image-async="{{image.thumbUrl || image.url}}" async-kind="thumb" >' +
 								//'<div class="thumb" ng-repeat="image in images track by image.id" ng-click="methods.open($index);" show-image-async="{{image.thumbUrl || image.url}}" async-kind="thumb" ng-style="{\'width\' : 480+\'px\', \'height\' : 332+\'px\'}">' +
